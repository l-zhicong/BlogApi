...
class Request extends \think\Request
{
     ...
     * @var array
     */
    protected $except = ['menu_path', 'api_url', 'unique_auth', 'description'];
    ...
    public function more(array $params, bool $suffix = false, bool $filter = true): array
    {
        ...
        foreach ($params as $param) {
            if (!is_array($param)) {
                $p[$suffix == true ? $i++ : $param] = $this->filterWord($this->param($param), $filter && !in_array($param, $this->except));
            } else {
                ...
                $p[$suffix == true ? $i++ : (isset($param[3]) ? $param[3] : $keyName)] = $this->filterWord($this->param($name, $param[1], $param[2]), $filter && !in_array($name, $this->except));
            }
        }
        return $p;
    }
